...
RegisterServerEvent('SEM_InteractionMenu:ReviveNear')
AddEventHandler('SEM_InteractionMenu:ReviveNear', function(ID)
	if ID == source then
		return
	end
	if ID == -1 or ID == '-1' then
		if source ~= '' then
			print('^1[#' .. source .. '] ' .. GetPlayerName(source) .. '  -  attempted to revive everyone?^7')
			DropPlayer(source, '\n[SEM_InteractionMenu] Attempting to Medical Revive Nearest Player')
		else
			print('^1 Someone tried to revive everyone^7')
		end

		return
	end
	
	TriggerClientEvent('SEM_InteractionMenu:Revive', ID, source)
end)
...
AddEventHandler('SEM_InteractionMenu:SeatNear', function(ID, player, Vehicle)
    TriggerClientEvent('SEM_InteractionMenu:Seat', ID, player, Vehicle)
end)
