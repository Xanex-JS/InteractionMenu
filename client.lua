...
RegisterNetEvent('StoreID')
AddEventHandler('StoreID', function(newID, NewDOB)
    CivName = newID
    CivDOB = NewDOB
end)
...
function DragNearestPlayer(ID)
    TriggerServerEvent("SEM_InteractionMenu:DragNear", ID, source)
end
  
exports('DragNearestPlayer', function(ID)
    TriggerServerEvent("SEM_InteractionMenu:DragNear", ID, source)
end)


function ForceOutOfCar(ID)
    TriggerServerEvent("SEM_InteractionMenu:UnseatNear", ID)
end
  
exports('ForceOutOfCar', function(ID)
    TriggerServerEvent("SEM_InteractionMenu:UnseatNear", ID)
end)

function ForceNearCar(ID)
    TriggerServerEvent("SEM_InteractionMenu:SeatNear", ID)
end
  
exports('ForceNearCar', function(ID)
    TriggerServerEvent("SEM_InteractionMenu:SeatNear", ID)
end)

function CuffNear(ID)
    TriggerServerEvent("SEM_InteractionMenu:CuffNear", ID)
end
  
exports('CuffNear', function(ID)
    TriggerServerEvent("SEM_InteractionMenu:CuffNear", ID)
end)

-- Revive Event

RegisterNetEvent('SEM_InteractionMenu:Revive')
AddEventHandler('SEM_InteractionMenu:Revive', function(ID)
	local ped = PlayerPedId()
	...
	if GetEntityHealth( ped ) <= 1 then 
		...
		ShowNotification("~r~You're in medical custody")
        ShowNotification("~r~Don't attempt to run away :/")
	end
end)
...
AddEventHandler('SEM_InteractionMenu:Seat', function(ID, player, Veh)

    if isCuffed and not Drag then 
...
AddEventHandler('SEM_InteractionMenu:JailPlayer', function(JailTime, PlayerID)
    ...
    if DoesEntityExist(Ped) then
        Citizen.CreateThread(function()
            ...
            while JailTime >= 0 and not PrisonBreak and not EarlyRelease do
                SetEntityInvincible(Ped, true)
                Citizen.Wait(5000)
                RemoveAllPedWeapons(Ped1)
                ...
                if JailTime % 30 == 0 and JailTime ~= 0 then
                    ...
                    sendMsg("~r~Don't spawn a ped otherwise you will glitch out and will need to relog")
				end
                
                Citizen.Wait(1000)

                local Location = GetEntityCoords(Ped1, true)
                ...
                local haskeycard1 = exports.AspireRP_PrisonBreak:GetKeycard()
				if Distance > 100 and haskeycard1 == true then
                    PrisonBreak = true
                    print("test")
					TriggerEvent('chat:addMessage', {
                        multiline = true,
                        color = {86, 96, 252},
                        args = {'Judge', 'You\'ve broken out of prison... the authoritys have been alerted :D'},
                    })
				end
                local Location = GetEntityCoords(Ped1, true)
				local Distance = Vdist(Config.JailLocation.Jail.x, Config.JailLocation.Jail.y, Config.JailLocation.Jail.z, Location['x'], Location['y'], Location['z'])
                local haskeycard1 = exports.AspireRP_PrisonBreak:GetKeycard()
                if Distance > 100 and haskeycard1 == false then
                    TriggerEvent("BrokeOutWithOutKeycard")
                    return
                end
            ...
            if PrisonBreak then
                ...
                TriggerServerEvent('SEM_InteractionMenu:GlobalChat', {255, 0, 0}, 'Federal Prison', GetPlayerName(PlayerId()) .. ' Has Broken out of Jail')
                ExecuteCommand("alarm_on")
            elseif CurrentlyJailed then 
...
RegisterNetEvent("BrokeOutWithOutKeycard")
AddEventHandler("BrokeOutWithOutKeycard", function(Ped, source)
    local Ped = GetPlayerPed(-1)
    RanOnce = true
    SetEntityCoords(Ped, Config.JailLocation.Jail.x, Config.JailLocation.Jail.y, Config.JailLocation.Jail.z)
    SetEntityHeading(Ped, Config.JailLocation.Jail.h)
    sendMsg("~r~You need a Keycard to break out")
    -- RanOnce = false
end)
...
AddEventHandler('SEM_InteractionMenu:HospitalizePlayer', function(HospitalTime, HospitalLocation, PlayerID)
    if CurrentlyHospitaled then
        return
    end
    if CurrentlyJailed then
    ...
    if DoesEntityExist(Ped) then
        Citizen.CreateThread(function()
            ...
            SetEntityCoords(Ped, HospitalLocation.Hospital.x, HospitalLocation.Hospital.y, HospitalLocation.Hospital.z)--{x = 459.5500793457,y = -994.46508789063,z = 23.914855957031 },

            while HospitalTime >= 0 and not EarlyDischarge do
                SetEntityInvincible(Ped, true)
                ExecuteCommand("e passout4", PlayerID)
                FreezeEntityPosition(Ped, PlayerID)
                if IsPedInAnyVehicle(Ped, false) then
					ClearPedTasksImmediately(Ped)
                end
                -- Draw3D Text
...
Citizen.CreateThread(function()
	while true do 
		Wait(0);
		local Ped = GetPlayerPed(-1);
		if CurrentlyHospitalised(Ped) then 
			Draw2DText(.5, .3, "~r~You're Currently Hospitalized...", 1.0, 1);
			Draw2DText(.5, .4, "~b~If You Try To Escape You Will Be Punished For FailRP", 1.0, 1);
        end
    end
end)

function CurrentlyHospitalised(PlayerID, Ped)
    if CurrentlyHospitaled then
    return true 
    else 
    return false
...
RegisterNetEvent('TogglePubMenu')
AddEventHandler('TogglePubMenu', function()
        PUBOnduty1 = not PUBOnduty1
        if PUBOnduty1 then
           -- Notify('~g~You are onduty as an LEO')
            exports["rp-radio"]:GivePlayerAccessToFrequencies(1, 2, 3, 4)
            PUBOnduty1 = true
...
function ToggleFireMenu(source)
    FireOnduty1 = not FireOnduty1
    if FireOnduty1 then
        FireAce = true
        FireOnduty = true
        exports["rp-radio"]:GivePlayerAccessToFrequencies(1, 2, 3, 4)
        Notify("~g~You've gone on duty as Medical")
    else
        Notify("~r~You've gone off duty as Medical")
        exports["rp-radio"]:RemovePlayerAccessToFrequencies(1, 2, 3, 4)
        FireAce = false
        FireOnduty = false
    end
...
exports('ToggleFireMenu', function(source)
    FireOnduty1 = not FireOnduty1
    if FireOnduty1 then
        FireAce = true
        FireOnduty = true
...
function TogglePublicMenu(source)
    PUBOnduty1 = not PUBOnduty1
    if PUBOnduty1 then
    PUBAce = true
    TriggerEvent("TogglePubMenu", source)
    else
    PUBAce = false
    end
end
  
exports('TogglePublicMenu', function(source)
    PUBOnduty1 = not PUBOnduty1
    if PUBOnduty1 then
    PUBAce = true
    TriggerEvent("TogglePubMenu", source)
...
function ToggleDuty(source)
    LEOOnduty1 = not LEOOnduty1
    if LEOOnduty1 then
    LEOAce = true
    TriggerEvent("ToggleMenu", source)
	print("Interaction")
    else
    LEOAce = false
    end
